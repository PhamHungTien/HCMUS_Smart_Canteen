...
const cancelOrder = (id) => {
    fetch(`/my-orders/${id}`, {
        method: 'PUT',
        headers: { 'Authorization': 'Basic ' + localStorage.getItem('auth') }
    }).then(res => res.json()).then(() => fetchMyOrders());
};
        ...
        {currentPage === 'settings' && (
            <div style={{maxWidth:400, margin:'80px auto'}}>
                ...
                <div className="card form-card" style={{marginBottom:20}}>
                    ...
                    {myOrders.length === 0 && <p>{t('no_orders')}</p>}
                    {myOrders.length > 0 && (
                        <table className="admin-table">
                            <thead>
                                <tr><th>ID</th><th>{t('pickup_time')}</th><th>{t('status')}</th><th></th></tr>
                            </thead>
                            <tbody>
                                {myOrders.map(o => (
                                    <tr key={o.id}>
                                        ...
                                        <td>{t('status_' + o.status)}</td>
                                        <td>
                                            {o.status === 'pending' && (
                                                <button className="btn danger-btn" onClick={() => cancelOrder(o.id)}>{t('cancel_order')}</button>
                                            )}
                                        </td>
                                    </tr>
                                ))}
